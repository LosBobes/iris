...
export function OrganizationNameSettings(): React.JSX.Element {
  ...
  const save = async (): Promise<void> => {
    ...
    try {
      ...
      toast.success("Naziv firme je sačuvan.");
    } catch (error) {
      toast.error(
        error instanceof Error && error.message.trim() !== ""
          ? `Greška pri čuvanju naziva: ${error.message}`
          : "Greška pri čuvanju naziva firme.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="max-w-2xl border border-border bg-card">
      <div className="flex items-center gap-2.5 border-b border-border px-5 py-4">
        ...
        <div>
          <div className="text-[13px] font-medium text-foreground">Naziv firme</div>
          <div className="text-[11px] text-[color:var(--iris-ink-soft)]">
            Prikazuje se u zaglavlju aplikacije za sve korisnike
          </div>
        </div>
      </div>
