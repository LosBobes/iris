...
interface SpotlightRect extends ViewportRect {
  x: number;
  y: number;
}

type TourStepPhase = "locating" | "entering" | "visible" | "exiting";

const TARGET_PADDING = 8;
const MOBILE_BREAKPOINT = 640;
const DESKTOP_CARD_WIDTH = 360;
const ESTIMATED_CARD_HEIGHT = 270;
const STEP_EXIT_DURATION_MS = 140;
...
function getSpotlightRect(element: HTMLElement): SpotlightRect {
  ...
  const bottom = Math.min(window.innerHeight - 8, rect.bottom + TARGET_PADDING);
...
function InteractiveTourOverlay({
  ...
  const isStepRendered = phase !== "locating";
    ...
    >
      ...
      >
        ...
        <rect
          width="100%"
          height="100%"
          fill={TOUR_BACKDROP_COLOR}
          mask="url(#iris-tour-mask)"
        />
      </svg>

      {targetRect && isStepRendered && (
        <>
          <div
            aria-hidden="true"
            ...
            style={{
              ...
              height: targetRect.height,
              borderRadius: 5,
              background: TOUR_BACKDROP_COLOR,
              opacity: isStepVisible ? 0 : 1,
              transitionDuration: opacityTransition.transitionDuration,
              transitionProperty: "opacity",
              transitionTimingFunction:
                opacityTransition.transitionTimingFunction,
            }}
          />
          <div
            aria-hidden="true"
        ...
        >
          ...
          <div className="mt-5 h-0.5 overflow-hidden bg-[color:var(--iris-border-soft)]">
            <div
              ...
              style={{
                width: `${((stepIndex + 1) / INTERACTIVE_TOUR_STEPS.length) * 100}%`,
              }}
            />
          </div>
...
export function InteractiveTourProvider({
  ...
  const clearStepTransition = useCallback(() => {
    if (stepTransitionTimerRef.current === null) return;
  ...
  }, [clearStepTransition, navigate, updatePhase]);
  ...
  const startTour = useCallback(() => {
    ...
    setTargetRect(null);
    setIsActive(true);
  }, [clearStepTransition, location.pathname, location.search, updatePhase]);
  ...
  useEffect(() => clearStepTransition, [clearStepTransition]);
  ...
  useEffect(() => {
    ...
    const waitForTargetToSettle = (
      ...
      if (prefersReducedMotion()) {
        settleFrameId = window.requestAnimationFrame(() => {
          ...
          if (currentTarget && revealTarget(currentTarget)) return;
          if (!currentTarget) {
      ...
      const measureTarget = (timestamp: number) => {
        ...
        if (elapsed >= TARGET_SETTLE_MAX_MS) {
          if (revealTarget(currentTarget)) return;
          ...
          settleFrameId = window.requestAnimationFrame(() => {
            const snappedTarget =
              findTarget(step) ??
              (currentTarget.isConnected ? currentTarget : null);
            if (snappedTarget && revealTarget(snappedTarget)) return;
            if (!snappedTarget) {
              retryId = window.setTimeout(
                locateTarget,
                TARGET_RETRY_INTERVAL_MS,
    ...
    function locateTarget(): void {
      ...
      waitForTargetToSettle(target, isMobile);
    }
    ...
    const updateTargetRect = () => {
      ...
      setTargetRect((currentRect) =>
        currentRect ? getSpotlightRect(target) : currentRect,
      );
    };
