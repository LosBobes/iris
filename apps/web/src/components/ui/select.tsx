...
function SelectContent({
  className,
  children,
  // Use Floating UI ("popper") rather than Radix's legacy "item-aligned"
  // positioning. Item-aligned computes its offset with manual getBoundingClientRect
  // math that ignores the app's `transform: scale` (font-scale accessibility mode),
  // so the menu lands detached from its trigger at larger scales. Popper mode is
  // scale-aware and anchors the menu to the trigger.
  position = "popper",
  align = "start",
  sideOffset = 4,
  ...props
