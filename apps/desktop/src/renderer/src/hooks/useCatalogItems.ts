...
/** Loads a page of catalog items through window.api (the preload bridge). */
export function useCatalogItems(
  query: CatalogItemQuery = {},
): UseCatalogItemsResult {
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const result = await window.api.getCatalogItems(
        JSON.parse(queryKey) as CatalogItemQuery,
      );
